...
export PGPOOL_PARAMS_AUTH_METHOD=${PGPOOL_PARAMS_AUTH_METHOD:-md5}
...
if [[ "$PGPOOL_PARAMS_AUTH_METHOD" = "md5" ]]; then
  enc_script="pg_md5"
elif [[ "$PGPOOL_PARAMS_AUTH_METHOD" = "scram-sha-256" ]]; then
  enc_script="pg_enc"
  echo 'two one nine two' > ~/.pgpoolkey
  chown postgres:postgres ~/.pgpoolkey
  chmod 600 ~/.pgpoolkey
else
  echo "ERROR: authentication method should be 'md5' or 'scram-sha-256'."
fi
...
if [[ -f ${PGPOOL_CONF_VOLUME}/pgpool.conf ]]; then
    ...
    if [[ "$PGPOOL_PARAMS_SSL" = "on" ]]; then
        ...
        echo "hostssl    all    all    0.0.0.0/0    ${PGPOOL_PARAMS_AUTH_METHOD}" >> ${PGPOOL_INSTALL_DIR}/etc/pool_hba.conf
    else

        # Setting pool_hba.conf
        echo "host    all    all    0.0.0.0/0    ${PGPOOL_PARAMS_AUTH_METHOD}" >> ${PGPOOL_INSTALL_DIR}/etc/pool_hba.conf
    fi
fi
...
do
    ...
    if [[ -n "${username}"  && -n "${password}" ]]; then
        ${PGPOOL_INSTALL_DIR}/bin/${enc_script} -m -f ${PGPOOL_INSTALL_DIR}/etc/pgpool.conf -u ${username} ${password}
